package com.cactus.netflixalpha.util;
...
public class CategoryTask extends AsyncTask <String, Void, List<Category>> {
    ...
    protected void onPreExecute() {
        ...
        if (context != null)
        dialog = ProgressDialog.show(context, "Carregando", "", true); // inderteminate - tempo que a progressDialog fica aberta
    }

    // thread Backgorund - roda de fundo a execução
    @Override
    protected List<Category> doInBackground(String... params) {
        ...
        try {
            ...
            BufferedInputStream in = new BufferedInputStream(inputStream); // Aloca memória pra manipular a stream em formato de buffer

            String jsonAsString  = bufferToString(in);
//            Log.i("Teste", jsonAsString);
    ...
    protected void onPostExecute(List<Category> categories) {
        ...
        // listener
        if(categoryLoader != null && categories != null)
            categoryLoader.onResult(categories);
    }


    private String bufferToString(InputStream in) throws IOException {
